...
for step in range(total_step):
    ...
    Q.eval()
    z_real_gauss = Variable(torch.randn(images.size()[0], z_red_dims) * 1.).to(device)
    D_real_gauss = D_gauss(z_real_gauss)

    z_fake_gauss = Q(images)
    D_fake_gauss = D_gauss(z_fake_gauss)

    D_acc = ((D_real_gauss > 0.5).sum() + (D_fake_gauss < 0.5).sum()) / (len(D_real_gauss) + len(D_fake_gauss) + EPS)
    ...
    if (step+1) % 100 == 0:
        print ('Step [%d/%d], Loss: %.4f, Acc: %.4f%%'
               %(step+1, total_step, D_loss.item(), D_acc*100))
